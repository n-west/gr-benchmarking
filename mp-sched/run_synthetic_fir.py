...
def write_shell_script(f, description, ncores, gflops, max_pipes_and_stages, database):
    ...
    for npipes in range(1, max_pipes_and_stages + 1):
        for nstages in range(1, max_pipes_and_stages + 1):
            ...
            f.write("echo 'Running synth FIR: %d pipes by %d stages'\n" % (npipes, nstages))
                                                                     
            cmd = "./%s/synthetic_fir.py -m -s %d -p %d -N %d -D %s \n" % (current_dir, nstages, npipes, nsamples, database)
            f.write(cmd)
...
"""
    usage = "usage: %prog [options] -D results.db -L label"
